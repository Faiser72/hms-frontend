...
export class PreliminaryComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //   this.preliminaryForm.patchValue({ patient: data.object })
    ...
    //   this.preliminaryForm.patchValue({ appointment: data.object })
    // })
    ...
    this.preliminaryFormBuilder();
    this.addTestReportFormBuilder()
  }

  preliminaryFormBuilder() {
    this.preliminaryForm = this.fb.group({
      ...
      appointment: "",
      diagnosisId: "",
      bodyMassIndex:[null, [Validators.required]],
      pulses:[null, [Validators.required]],
      heartBeatRate:[null, [Validators.required]],
  ...
  // preliminaryFormSubmit() {
  //   this.preliminaryForm.patchValue({ diagnosisId: this.diagnosisId });
  //   if (this.preliminaryForm.valid) {
  //     this.appComponent.startSpinner("Updating data..\xa0\xa0Please wait ...");
  //     this.patientDiagnosisService.updatePatientDiagnosisDetails(this.preliminaryForm.value).subscribe((data: any) => {
  //       if (data.success) {
  //         this.appComponent.stopSpinner();
  //         alert(data.message)
  //         setTimeout(() => {
